...
def test_mnist(feature_encoder, relation_network):
    # test
    print("MNIST Testing...")
    # total_rewards = 0
    acc_average = AverageMeter()

    db_mnist = MnistNShot(MNIST_ROOT,
                          batchsz=1,
                          n_way=CLASS_NUM,
                          k_shot=SAMPLE_NUM_PER_CLASS,
                          k_query=BATCH_NUM_PER_CLASS,
                          imgsz=28)
    ...
    for i in range(TEST_EPISODE):
        ...
        for x_spt_one, y_spt_one, x_qry_one, y_qry_one in zip(x_spt, y_spt, x_qry, y_qry):
            sample_images, sample_labels, test_images, test_labels = torch.from_numpy(x_spt_one).to(device), torch.from_numpy(y_spt_one).to(device), \
                                                                     torch.from_numpy(x_qry_one).to(device), torch.from_numpy(y_qry_one).to(device)

            # calculate features
            sample_features = feature_encoder(sample_images)  # 5x64
            sample_features = sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 5, 5)
            sample_features = torch.sum(sample_features, 1).squeeze(1)
            test_features = feature_encoder(test_images)  # 20x64
            ...
            rewards = [1 if predict_labels[j] == test_labels[j] else 0 for j in range(CLASS_NUM * SAMPLE_NUM_PER_CLASS)]
            acc_average.update(np.sum(rewards) / (CLASS_NUM * SAMPLE_NUM_PER_CLASS))
...
def main():
    ...
    #     feature_encoder.load_state_dict(torch.load(str("./models/omniglot_feature_encoder_" + str(CLASS_NUM) + "way_" + str(SAMPLE_NUM_PER_CLASS) + "shot.pkl"), map_location=torch.device(device)))
    #     print("load feature encoder success")
    # if os.path.exists(str("./models/omniglot_relation_network_" + str(CLASS_NUM) + "way_" + str(SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
    #     relation_network.load_state_dict(torch.load(str("./models/omniglot_relation_network_" + str(CLASS_NUM) + "way_" + str(SAMPLE_NUM_PER_CLASS) + "shot.pkl"), map_location=torch.device(device)))
    ...
    for episode in range(EPISODE):
        ...
        sample_features = sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 5, 5)
        sample_features = torch.sum(sample_features, 1).squeeze(1)
        batch_features = feature_encoder(Variable(batches).to(device))  # 20x64*5*5
    ...
    # test omniglot at the end
    test(feature_encoder, relation_network)

    # test mnist at the end
    test_mnist(feature_encoder, relation_network)
